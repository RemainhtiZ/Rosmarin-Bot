...
const roomResSvg=(res, allCnt, len)=>{
    ...
    let percentText = `<text x="${len + 10}" y="10" font-size="10" fill="${RR_COLORS.text}">${(left/allCnt*100).toFixed(1)}%</text>`
    let capacityText = `<text x="${len + 60}" y="10" font-size="10" fill="${RR_COLORS.textMuted}">${(allCnt/1000000).toFixed(1)}M</text>`
    
    let exist = allCnt ? 
        `<rect width="${len}" height="12" fill="${RR_COLORS.bgLight}" rx="2" ry="2"/>
         ${svgs}
         ${percentText}
         ${capacityText}` : "";
...
return`
    <t class="${divName}" onclick="gotoRoom('${roomName}')" style="color:${RR_COLORS.theme};cursor:pointer;font-weight:bold;padding:2px 6px;border-radius:3px;background:${RR_COLORS.bgHeader};border:1px solid ${RR_COLORS.border};">[${roomName}]</t><script>
    (() => {
        ...
        button.addEventListener("pointerenter", () => {
            ...
            tip = document.createElement("div");
                tip.style.backgroundColor = "${RR_COLORS.bgDark}"; 
                tip.style.border = "1px solid ${RR_COLORS.border}";
                tip.style.borderRadius = "8px";
            tip.style.position = "absolute";
            tip.style.zIndex=10;
                tip.style.color = "${RR_COLORS.text}";
            tip.style.marginLeft = "0px";
                tip.style.boxShadow = "0 4px 6px rgba(0,0,0,0.3)";
                tip.innerHTML = '<div id="${divNameShow}" onclick="" style="height: 600px;width:600px;color:${RR_COLORS.text}"/>';
            button.append(tip);
...
function allResTips(text, tipStrArray, id, left){
    left = left-1;
    left*=100;
    let showCore = tipStrArray.map(e=>`<div style="padding:2px 5px;margin:0;background:${RR_COLORS.bgLight};border:1px solid ${RR_COLORS.border};border-radius:4px;font-size:12px;line-height:1.15;"><t onclick="goto('${e}')" style="cursor:pointer;"> ${e} </t></div>`.replace(/[\\"]/g,'%')).join("")
    let time = Game.time;
    return `<t class="a${id}-a${time}" style="cursor:pointer;display:inline-flex;align-items:center;padding:1px 5px;border-radius:3px;transition:all 0.2s;font-size:12px;line-height:1.15;">${text}</t><script>
    function goto(e){
        ...
        let tip;
        button.addEventListener("pointerenter", () => {
                button.style.background = "${RR_COLORS.bgLight}";
            tip = document.createElement("div");
                tip.style.backgroundColor = "${RR_COLORS.bgDark}"; 
                tip.style.border = "1px solid ${RR_COLORS.border}";
...
let pro = {
    ...
    showAllRes(){
        ...
        <div class="rr-container">
            <style>
                .rr-container{white-space:normal;font-family:Consolas,monospace;background-color:${RR_COLORS.bgDark};padding:10px;border-radius:6px;margin-top:8px;color:${RR_COLORS.text};border:1px solid ${RR_COLORS.border};box-shadow:0 4px 8px rgba(0,0,0,0.28)}
                .rr-title{font-size:14px;margin-bottom:6px;display:flex;align-items:center;color:${RR_COLORS.theme};font-weight:700;text-transform:uppercase;letter-spacing:1px;border-left:3px solid ${RR_COLORS.theme};padding-left:8px;padding-bottom:0}
                .rr-cell{border-radius:4px;padding:4px 6px;display:flex;justify-content:space-between;align-items:center;line-height:1.15;font-size:12px;border:1px solid ${RR_COLORS.border};background-color:${RR_COLORS.bgLight};box-sizing:border-box;min-height:24px}
                .rr-label{display:inline-flex;align-items:center;gap:2px;padding:0 !important;margin:0 !important}
                .rr-number{font-weight:600;text-align:right;min-width:52px;font-variant-numeric:tabular-nums}
                .rr-mute{opacity:.65}
            </style>
            <div class="rr-title">
                💎资源总览
        ...
        let id = 0
        let categoryStyle = `background-color: ${RR_COLORS.bgHeader}; color: ${RR_COLORS.text}; font-weight: 700; padding: 6px 8px; border-radius: 6px; margin: 8px 0 4px 0; display: block; font-size: 11px; letter-spacing: 1px; text-transform: uppercase; border: 1px solid ${RR_COLORS.border};`;
        
        // 创建资源单元格
        let createResourceCell = function(resType, amount) {
            const icon = getResourceIcon(resType);
            const bgColor = amount > 0 ? RR_COLORS.bgLight : RR_COLORS.bgHeader;
            const textColor = amount > 0 ? (RES_COLOR_MAP[resType] || '#ccc') : RR_COLORS.textMuted;
            ...
            const tipContent = arr.length > 0 ? 
                ...
                `<t style="display:inline-flex;align-items:center;padding:1px 5px;border-radius:3px;font-size:12px;line-height:1.15;">${labelHtml}</t>`;
        ...
        let addResourceTable = function(resourceList, title) {
            ...
            if (filteredList.length === 0) {
                html += `
                <div style="padding: 8px; text-align: center; color: ${RR_COLORS.textMuted}; font-style: italic; background-color: ${RR_COLORS.bgLight}; border: 1px solid ${RR_COLORS.border}; border-radius: 6px; margin-bottom: 10px; font-size: 12px;">
                    此类别暂无资源
                </div>`;
                return;
            }
            ...
            for (let i = 0; i < filteredList.length; i += itemsPerRow) {
                html += `<div style="display: grid; grid-template-columns: repeat(${itemsPerRow}, 1fr); gap: 4px; margin-bottom: 4px;">`;
        ...
        <div style="margin-bottom: 6px;">
            <div style="display: flex; margin-bottom: 4px; border-bottom: 1px solid ${RR_COLORS.border}; padding-bottom: 4px; font-size: 12px;">
                ...
                <div style="flex: 1; text-align: center; color: ${RR_COLORS.text};">白色(G)</div>
            </div>
    ...
    showRoomRes(){
        ...
        <div style="white-space: normal; font-family: Consolas, monospace; background-color: ${RR_COLORS.bgDark}; padding: 10px; border-radius: 8px; margin-top: 10px; color: ${RR_COLORS.text}; border: 1px solid ${RR_COLORS.border}; box-shadow: 0 4px 8px rgba(0,0,0,0.3);">
            <div style="font-size: 14px; margin-bottom: 8px; color: ${RR_COLORS.theme}; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; border-left: 3px solid ${RR_COLORS.theme}; padding-left: 8px;">
                🏠房间资源概览
            </div>
        `;
        ...
        roomData.forEach(data => {
            ...
            html += `
            <div style="background-color: ${RR_COLORS.bgLight}; border: 1px solid ${RR_COLORS.border}; border-radius: 8px; padding: 10px; display: flex; flex-direction: column; gap: 8px;">
                <div style="display: flex; align-items: center; justify-content: space-between;">
                    ...
                    <div style="display: flex; gap: 10px;">
                        <div>
                            <span style="color: ${RR_COLORS.textMuted}; font-size: 12px;">总资源: </span>
                            <span style="color: ${RR_COLORS.text};">${(data.totalResources/1000).toFixed(1)}K</span>
                        </div>
                        <div>
                            <span style="color: ${RR_COLORS.textMuted}; font-size: 12px;">剩余: </span>
                ...
                <div>
                    <div style="display: flex; align-items: center; margin-bottom: 4px;">
                        <span style="width: 70px; color: ${RR_COLORS.textMuted}; font-size: 12px;">Storage: </span>
                        ${roomResSvg(data.storage, data.storageCap, 500)}
                    </div>
                    <div style="display: flex; align-items: center;">
                        <span style="width: 70px; color: ${RR_COLORS.textMuted}; font-size: 12px;">Terminal: </span>
        ...
        html += `
            ...
            <div style="font-size: 10px; color: ${RR_COLORS.textMuted}; text-align: right; margin-top: 8px; padding-top: 8px; border-top: 1px solid ${RR_COLORS.border};">
